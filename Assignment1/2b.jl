...
function costFunction(X, Y, C,lambda)
    ...
    m = length(Y)
    cost = sum(((X * C) - Y).^2)/(2*m) + lambda*(sum(sumB))/4
    return cost
end
...
function gradientDescent(X, Y, B, learningRate, numIterations,lambda)
    ...
    for iteration in 1:numIterations
        ...
        gradient = (X' * loss)/m + (lambda/4)*signB;
...
lambda=-400000
error=zeros(1000,1)
lambdaa=zeros(1000,1)
min_er=5000000000000

for i in 1:1000
lambda[i]=alfa
newB, _,_, _ = gradientDescent(X_val, Y_val, B, 0.001, 500,lambda)
...
newB, costHistory, error, score = gradientDescent(X_train, Y_train, B, learning_rate, 850,lambdaa[index])
